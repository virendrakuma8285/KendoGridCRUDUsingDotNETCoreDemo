...
var viewFactory = function (rows, className) {
    var viewElement;
    return function () {
        if (!util_1.isDocumentAvailable) {
            return null;
        ...
        if (!viewElement) {
            viewElement = view(content(rows), className);
        }
        return viewElement;
    };
};
var monthView = viewFactory(6, 'k-calendar-view k-calendar-monthview');
var yearView = viewFactory(3, 'k-calendar-view k-calendar-yearview');
var decadeView = viewFactory(2, 'k-calendar-view k-calendar-decadeview');
...
var DOMService = (function () {
    ...
    DOMService.prototype.calculateHeights = function (container) {
        ...
        this.batch(monthView(), function (contentElement) {
            var viewElement = contentElement.querySelector('tbody');
            ...
            _this.scrollableContentHeight = height(contentElement.querySelector('.k-scrollable'));
        });
        this.batch(yearView(), function (contentElement) {
            _this.yearViewHeight = height(contentElement.querySelector('tbody'));
            _this.scrollableYearContentHeight = height(contentElement.querySelector('.k-scrollable')) + _this.headerHeight;
        });
        this.batch(decadeView(), function (contentElement) {
            _this.decadeViewHeight = height(contentElement.querySelector('tbody'));
